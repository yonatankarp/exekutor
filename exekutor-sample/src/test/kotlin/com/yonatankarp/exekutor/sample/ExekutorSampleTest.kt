...
class ExekutorSampleTest {
    ...
    @Test
    fun `should approve a clean user`() = runTest {
        // Given
        val engine = engine()
    ...
    @Test
    fun `should fail if fraud is detected`() = runTest {
        // Given
        val engine = engine()
    ...
    @Test
    fun `should require friction for low credit score`() = runTest {
        // Given
        val engine = engine()
    ...
    private fun engine(): StepExecutionEngine<RiskContext> =
        StepExecutionEngine(planBuilder = { listOf(FraudCheck(), CreditCheck()) })

    private fun context(payload: String): RiskContext =
        RiskContext(payload = payload, timeBudgetMs = 3000)
